...
@interface RTC_OBJC_TYPE (POCEAGLVideoView)
()<GLKViewDelegate>
...
@implementation RTC_OBJC_TYPE (POCEAGLVideoView) {
  RTCDisplayLinkTimer *_timer;
  EAGLContext *_glContext;
  // This flag should only be set and read on the main thread (e.g. by
  // setNeedsDisplay)
  BOOL _isDirty;
  id<RTC_OBJC_TYPE(POCVideoViewShading)> _shader;
  POCNV12TextureCache *_nv12TextureCache;
...
- (instancetype)initWithFrame:(CGRect)frame {
  return [self initWithFrame:frame shader:[[POCDefaultShader alloc] init]];
}

- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  return [self initWithCoder:aDecoder shader:[[POCDefaultShader alloc] init]];
}

- (instancetype)initWithFrame:(CGRect)frame shader:(id<RTC_OBJC_TYPE(POCVideoViewShading)>)shader {
...
- (BOOL)configure {
  ...
  __weak RTC_OBJC_TYPE(POCEAGLVideoView) *weakSelf = self;
  _timer = [[RTCDisplayLinkTimer alloc] initWithTimerHandler:^{
    RTC_OBJC_TYPE(POCEAGLVideoView) *strongSelf = weakSelf;
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
  ...
  if ([frame.buffer isKindOfClass:[RTC_OBJC_TYPE(RTCCVPixelBuffer) class]]) {
      ...
      CGFloat height = CVPixelBufferGetHeight(buffer.pixelBuffer);
      if (!_nv12TextureCache) {
      _nv12TextureCache = [[POCNV12TextureCache alloc] initWithContext:_glContext];
    }
    if (_nv12TextureCache) {
      [_nv12TextureCache uploadFrameToTextures:frame];
        // The pixel format should be RGBA. So uvPlane won't pass texture.
      [_shader applyShadingForFrameWithWidth:frame.width
                                      height:frame.height
                                    rotation:rotation
                                     texture:_nv12TextureCache.texture];
